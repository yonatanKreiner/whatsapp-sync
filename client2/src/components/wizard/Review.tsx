...
const Review = () => {
  ...
  const whatsappContacts =
    sessionStorageUtil.getItem<any[]>("WHATSAPP_CONTACTS");
  const googleContacts = sessionStorageUtil.getItem<any[]>("GOOGLE_CONTACTS");
  ...
  const updateContacts = async () => {
    setIsLoading(true);
    try {
      const res = await UpdateContactsProfilesPhotos(
        contacts.map((x) => ({
  ...
  const generateCards = () => {
    if (contacts) {
      const peopleCards = contacts.map((p) => (
        ...
        >
          <h4 style={{ textAlign: "center", margin: "10px 5px" }}>
            {p.gContact.name}
          </h4>
          <span>
            <img
              style={{ borderRadius: "50%" }}
              src={p.gContact.imageUrl ?? ""}
            ></img>
            <h5 style={{ fontWeight: "bold", textAlign: "center" }}>&dArr;</h5>
            <img
              style={{ borderRadius: "50%" }}
              src={p.wContact.imageURL ?? ""}
            ></img>
          </span>
        </div>
      ));
  ...
  return (
    <div>
      <div className="text-center mb-8">
        <h1 className="text-2xl md:text-3xl font-bold mb-4">Review & Import</h1>
      </div>
